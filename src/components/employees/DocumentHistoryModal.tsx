  ...
  return (
    ...
    >
      <div
        ...
        className="relative bg-white dark:bg-slate-900 rounded-xl shadow-2xl w-full max-w-6xl max-h-[92vh] flex flex-col border border-slate-200 dark:border-slate-700 overflow-hidden"
      >
        {/* Modal Header */}
        <div
        ...
        >
          ...
          <div className="flex items-center gap-2">
            ...
            >
              {showMasked ? <EyeOff className="w-3.5 h-3.5 text-amber-400" /> : <Eye className="w-3.5 h-3.5 text-slate-400 dark:text-slate-500" />}
            ...
            <button
              ...
              type="button"
              onClick={onClose}
              className="p-1.5 text-slate-400 dark:text-slate-500 hover:text-white hover:bg-slate-800 rounded-lg transition-colors ml-2"
        ...
        <div
          ...
          className="px-6 py-2.5 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700 flex items-center gap-2 overflow-x-auto shrink-0 scrollbar-thin"
        >
          <span className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mr-1 shrink-0">Document Type:</span>
          {(
            [
              ...
              >
                ...
                <span
                  className={`px-1.5 py-0.2 rounded-full text-[10px] font-bold ${
                    isActive ? 'bg-slate-700 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                  }`}
                >
                  {count}
                </span>
        ...
        <div
          ...
          className="px-6 py-3 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3 shrink-0"
        >
          ...
          <div className="flex items-center gap-3 flex-1 min-w-[260px] max-w-lg">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-slate-900 focus:bg-white transition-all"
              />
              {searchQuery && (
                <button
                  type="button"
            ...
            <select
              ...
              className="px-3 py-1.5 text-xs bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-700 dark:text-slate-300 font-medium focus:outline-none focus:ring-2 focus:ring-slate-900"
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-semibold rounded-lg border flex items-center gap-1.5 transition-all ${
                compareMode
                  ? 'bg-indigo-50 dark:bg-indigo-900/30 border-indigo-300 dark:border-indigo-700 text-indigo-700 dark:text-indigo-300 shadow-xs'
            ...
            >
              ...
              {compareMode && (
                <span className="px-1.5 py-0.2 bg-indigo-200 dark:bg-indigo-900/50 text-indigo-800 dark:text-indigo-300 rounded-full text-[10px]">
        ...
        <div id="document-history-body" className="p-6 overflow-y-auto flex-1 space-y-6 bg-slate-50/50">
          {/* Error Message */}
          {error && (
            <div className="p-4 bg-rose-50 dark:bg-rose-900/30 border border-rose-200 dark:border-rose-800/60 rounded-xl text-rose-800 dark:text-rose-300 text-xs flex items-center gap-3">
              ...
              <button
                type="button"
                onClick={fetchHistoryData}
                className="px-2 py-1 bg-rose-100 dark:bg-rose-900/40 hover:bg-rose-200 rounded font-semibold text-rose-900 dark:text-rose-300 text-xs"
              >
                Retry
              </button>
            </div>
          ...
          {!loading && (
            <div
              ...
              className="bg-white dark:bg-slate-900 rounded-xl p-4 border border-slate-200 dark:border-slate-700 shadow-xs grid grid-cols-2 sm:grid-cols-4 gap-4"
            >
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-100 dark:border-slate-800">
                ...
                <div className="mt-1 flex items-baseline gap-2">
                  <span className="text-2xl font-bold text-slate-900 dark:text-slate-100">{metrics.totalVersions}</span>
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 rounded-lg border border-slate-100 dark:border-slate-800">
                ...
                <div className="mt-1.5">
                  {metrics.activeDoc ? (
                    ...
                    <span className="text-xs font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/30 px-2 py-0.5 rounded border border-amber-200 dark:border-amber-800/60">
                      No Active Version
                    </span>
                  )}
                </div>
          ...
          {compareMode && (
            <div
              ...
              className="bg-indigo-50/70 border-2 border-indigo-200 dark:border-indigo-800/60 rounded-xl p-5 shadow-xs space-y-4"
            >
              ...
              {selectedForCompare.length === 2 && compareItems.length === 2 ? (
                <div className="bg-white dark:bg-slate-900 rounded-lg border border-indigo-200 dark:border-indigo-800/60 overflow-hidden shadow-xs">
                  ...
                  <div className="divide-y divide-slate-100 dark:divide-slate-800 text-xs">
                    {/* Document Number */}
                    <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="font-semibold text-slate-500 dark:text-slate-400">Document Number</span>
                      <span className="font-mono font-medium text-slate-900 dark:text-slate-100">{maskValue(compareItems[0].documentNumber)}</span>
                      <span className="font-mono font-medium text-slate-900 dark:text-slate-100">{maskValue(compareItems[1].documentNumber)}</span>
                    </div>

                    {/* Status */}
                    <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="font-semibold text-slate-500 dark:text-slate-400">Lifecycle Status</span>
                      <div>
                        <span className={`inline-flex px-2 py-0.5 rounded text-[11px] font-bold ${compareItems[0].isCurrent ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                          {compareItems[0].isCurrent ? 'Current Active' : 'Superseded'}
                        </span>
                      </div>
                      <div>
                        <span className={`inline-flex px-2 py-0.5 rounded text-[11px] font-bold ${compareItems[1].isCurrent ? 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300'}`}>
                    ...
                    {/* Expiry Date */}
                    <div className={`grid grid-cols-3 p-3 ${compareItems[0].expiryDate !== compareItems[1].expiryDate ? 'bg-amber-50/60' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                      <span className="font-semibold text-slate-500 dark:text-slate-400 flex items-center gap-1.5">
                        ...
                        {compareItems[0].expiryDate !== compareItems[1].expiryDate && (
                          <span className="text-[10px] bg-amber-200 dark:bg-amber-900/50 text-amber-900 dark:text-amber-300 px-1.5 rounded font-bold">Modified</span>
                        )}
                      </span>
                      <span className="font-medium text-slate-800 dark:text-slate-200">{formatDate(compareItems[0].expiryDate)}</span>
                      <span className="font-medium text-slate-800 dark:text-slate-200">{formatDate(compareItems[1].expiryDate)}</span>
                    </div>

                    {/* Issue Date */}
                    <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="font-semibold text-slate-500 dark:text-slate-400">Issue Date</span>
                      <span className="text-slate-800 dark:text-slate-200">{formatDate(compareItems[0].issueDate)}</span>
                      <span className="text-slate-800 dark:text-slate-200">{formatDate(compareItems[1].issueDate)}</span>
                    </div>

                    {/* Authority */}
                    <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="font-semibold text-slate-500 dark:text-slate-400">Issuing Authority</span>
                      <span className="text-slate-800 dark:text-slate-200">{compareItems[0].issuingAuthority || '—'}</span>
                      <span className="text-slate-800 dark:text-slate-200">{compareItems[1].issuingAuthority || '—'}</span>
                    </div>

                    {/* Special fields (Trade / Category) */}
                    {(compareItems[0].tradeOnVisa || compareItems[1].tradeOnVisa) && (
                      <div className={`grid grid-cols-3 p-3 ${compareItems[0].tradeOnVisa !== compareItems[1].tradeOnVisa ? 'bg-amber-50/60' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>
                        <span className="font-semibold text-slate-500 dark:text-slate-400">Trade on Visa</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{compareItems[0].tradeOnVisa || '—'}</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{compareItems[1].tradeOnVisa || '—'}</span>
                      </div>
                    )}

                    {(compareItems[0].licenceCategory || compareItems[1].licenceCategory) && (
                      <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                        <span className="font-semibold text-slate-500 dark:text-slate-400">Licence Category</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{compareItems[0].licenceCategory || '—'}</span>
                        <span className="font-semibold text-slate-800 dark:text-slate-200">{compareItems[1].licenceCategory || '—'}</span>
                      </div>
                    )}

                    {/* Replacement Reason */}
                    <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="font-semibold text-slate-500 dark:text-slate-400">Renewal / Change Reason</span>
                      <span className="text-slate-700 dark:text-slate-300 italic">{compareItems[0].replaceReason || 'Initial registration'}</span>
                      <span className="text-slate-700 dark:text-slate-300 italic">{compareItems[1].replaceReason || 'Initial registration'}</span>
                    </div>

                    {/* Attachment comparison */}
                    <div className="grid grid-cols-3 p-3 hover:bg-slate-50 dark:hover:bg-slate-800">
                      <span className="font-semibold text-slate-500 dark:text-slate-400">Attachment Copy</span>
                      <div>
                ...
                <div className="bg-white/80 p-6 text-center rounded-lg border border-dashed border-indigo-200 dark:border-indigo-800/60 text-xs text-indigo-700 dark:text-indigo-300">
          ...
          {loading ? (
            <div className="space-y-4 py-10">
              <div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500 space-y-3">
                <RefreshCw className="w-8 h-8 animate-spin text-slate-600 dark:text-slate-400" />
                <span className="text-xs font-semibold">Loading document audit trail...</span>
              </div>
            </div>
          ) : filteredItems.length === 0 ? (
            <div className="bg-white dark:bg-slate-900 rounded-xl p-10 text-center border border-slate-200 dark:border-slate-700 space-y-3">
              ...
              <h4 className="text-sm font-bold text-slate-800 dark:text-slate-200">No Document Records Found</h4>
              ...
              {searchQuery && (
                <button
                  type="button"
                  onClick={() => setSearchQuery('')}
                  className="px-3 py-1.5 text-xs font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-300 rounded-lg"
                >
                  Clear Filters
                </button>
              )}
            ...
            <div id="document-history-timeline" className="space-y-4 relative">
              ...
              <div className="absolute left-6 top-6 bottom-6 w-0.5 bg-slate-200 dark:bg-slate-700 hidden sm:block -z-0" />
                ...
                return (
                  <div
                    ...
                    className={`relative rounded-xl transition-all border ${
                      item.isCurrent
                        ? 'bg-white dark:bg-slate-900 border-emerald-300 dark:border-emerald-700 ring-2 ring-emerald-500/20 shadow-md'
                        : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 shadow-xs hover:border-slate-300'
                    } ${isSelectedForDiff ? 'ring-2 ring-indigo-500 bg-indigo-50/20' : ''}`}
                  >
                    {/* Header bar */}
                    <div className="p-4 sm:p-5 flex flex-wrap items-start justify-between gap-3 border-b border-slate-100 dark:border-slate-800">
                      <div className="flex items-start gap-3.5">
                        ...
                        <div>
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {item.isCurrent ? (
                              <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-emerald-100 dark:bg-emerald-900/40 text-emerald-800 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-800/60 flex items-center gap-1">
                                <CheckCircle2 className="w-3 h-3" />
                                Current Active Version
                              </span>
                            ) : (
                              <span className="px-2 py-0.5 rounded text-[11px] font-semibold bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200 dark:border-slate-700 flex items-center gap-1">
                        ...
                        {(item.fileName || item.documentAttachment) && (
                          <>
                            <button
                              ...
                              className="px-2.5 py-1.5 text-xs font-semibold bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-800 dark:text-slate-200 rounded-lg flex items-center gap-1.5 transition-colors border border-slate-200 dark:border-slate-700"
                            ...
                            {item.documentAttachment && (
                              <a
                                ...
                                className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-800 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg border border-slate-200 dark:border-slate-700 transition-colors"
                                title="Download File"
                              >
                    ...
                    <div className="p-4 sm:p-5 space-y-4">
                      ...
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs bg-slate-50/80 p-3 rounded-lg border border-slate-100 dark:border-slate-800">
                        <div>
                          <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Issue Date</span>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{formatDate(item.issueDate)}</span>
                        </div>

                        <div>
                          <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Expiry Date</span>
                          <span className="font-semibold text-slate-800 dark:text-slate-200">{formatDate(item.expiryDate)}</span>
                        </div>

                        {item.tradeOnVisa && (
                          <div>
                            <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Trade on Visa</span>
                            <span className="font-bold text-slate-900 dark:text-slate-100">{item.tradeOnVisa}</span>
                          </div>
                        )}

                        {item.sponsor && (
                          <div>
                            <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Visa Sponsor</span>
                            <span className="font-semibold text-slate-800 dark:text-slate-200">{item.sponsor}</span>
                          </div>
                        )}

                        {item.licenceCategory && (
                          <div>
                            <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Licence Category</span>
                            <span className="font-bold text-slate-900 dark:text-slate-100">{item.licenceCategory}</span>
                          </div>
                        )}

                        {item.vehicleClass && (
                          <div>
                            <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Vehicle Class</span>
                            <span className="font-semibold text-slate-800 dark:text-slate-200">{item.vehicleClass}</span>
                          </div>
                        )}

                        {item.bloodGroup && (
                          <div>
                            <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Blood Group</span>
                            <span className="font-bold text-rose-700 dark:text-rose-300">{item.bloodGroup}</span>
                          </div>
                        )}

                        {item.effectiveFrom && (
                          <div>
                            <span className="text-slate-400 dark:text-slate-500 block text-[11px]">Effective Window</span>
                      ...
                      {/* Audit stamp footer */}
                      <div className="flex flex-wrap items-center justify-between gap-2 text-[11px] text-slate-400 dark:text-slate-500 pt-1 border-t border-slate-100 dark:border-slate-800">
                        <div className="flex items-center gap-3">
                          {item.createdBy && (
                            <span>Recorded by: <strong className="text-slate-600 dark:text-slate-400">{item.createdBy}</strong></span>
                          )}
                          {item.createdAt && (
                            <span>Created: <strong className="text-slate-600 dark:text-slate-400">{formatDate(item.createdAt)}</strong></span>
                          )}
                          {item.updatedAt && (
                            <span>Last audit: <strong className="text-slate-600 dark:text-slate-400">{formatDate(item.updatedAt)}</strong></span>
                          )}
                        </div>
        ...
        <div
          ...
          className="px-6 py-3.5 bg-slate-100 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-3 shrink-0"
        >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              type="button"
              onClick={onClose}
              className="px-4 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-lg transition-colors shadow-xs"
            >
              Close
            </button>
          </div>
