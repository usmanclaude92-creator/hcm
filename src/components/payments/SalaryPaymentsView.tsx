...
export const SalaryPaymentsView: React.FC = () => {
  ...
  const renderSortIcon = (column: SalaryPaymentSortColumn) => {
    ...
    return sortDirection === 'asc' ? (
      <ArrowUp className="w-3 h-3 text-emerald-600 dark:text-emerald-400 ml-1 shrink-0 font-bold" />
    ) : (
      <ArrowDown className="w-3 h-3 text-emerald-600 dark:text-emerald-400 ml-1 shrink-0 font-bold" />
    );
  };
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-wrap items-center gap-2.5">
          <div className="flex items-center gap-2 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg px-3 py-1.5 shadow-2xs">
            ...
            <input
              ...
              className="text-xs font-semibold text-slate-800 dark:text-slate-200 focus:outline-hidden disabled:opacity-40"
            />
            <label className="flex items-center gap-1 text-[11px] text-slate-500 dark:text-slate-400 font-medium pl-1 border-l border-slate-200 dark:border-slate-700 cursor-pointer">
      ...
      {summary && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-rose-200 dark:border-rose-800/60 bg-rose-50/30 shadow-xs">
            <span className="text-xs font-semibold text-rose-700 dark:text-rose-300">Outstanding Salary</span>
            <strong className="block text-xl font-bold text-rose-800 dark:text-rose-300 mt-1 font-mono">
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs">
            <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Unpaid Lines</span>
            <strong className="block text-xl font-bold text-slate-900 dark:text-slate-100 mt-1 font-mono">{summary.unpaidCount}</strong>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-amber-200 dark:border-amber-800/60 bg-amber-50/30 shadow-xs">
            <span className="text-xs font-semibold text-amber-700 dark:text-amber-300">Partially Paid</span>
            <strong className="block text-xl font-bold text-amber-800 dark:text-amber-300 mt-1 font-mono">{summary.partiallyPaidCount}</strong>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-emerald-200 dark:border-emerald-800/60 bg-emerald-50/30 shadow-xs">
            <span className="text-xs font-semibold text-emerald-700 dark:text-emerald-300">Total Paid</span>
          ...
          <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-blue-200 dark:border-blue-800/60 bg-blue-50/30 shadow-xs">
            <span className="text-xs font-semibold text-blue-700 dark:text-blue-300 flex items-center gap-1"><Clock className="w-3 h-3" /> Receipts Pending</span>
            <strong className="block text-xl font-bold text-blue-800 dark:text-blue-300 mt-1 font-mono">{summary.pendingReceiptsCount}</strong>
          </div>
        </div>
      )}
      ...
      <div className="bg-white dark:bg-slate-900 p-3 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs space-y-3">
        <div className="relative w-full">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500"
          />
          {search && (
            <button
              type="button"
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pb-1 border-b border-slate-100 dark:border-slate-800">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <span className="text-xs font-bold text-slate-800 dark:text-slate-200 uppercase tracking-wider">Payment Filters</span>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-medium">
              (Showing {displayRows.length} records)
            </span>
            {summary && (
              <>
                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs font-semibold text-slate-700 dark:text-slate-300">
                  Net: OMR {formatOMR(summary.totalNetSalary)}
                </span>
                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md bg-emerald-50 dark:bg-emerald-900/30 border border-emerald-200 dark:border-emerald-800/60 text-xs font-semibold text-emerald-700 dark:text-emerald-300">
          ...
          <div className="flex items-center gap-3">
            <span className="text-[11px] text-slate-500 dark:text-slate-400 font-medium hidden md:inline">
              Sort: <span className="font-semibold text-emerald-600 dark:text-emerald-400">{sortColumn === 'default' ? 'Type (Staff/Worker) → Company (ASC) → Emp Code (ASC) → Month (DESC)' : `${sortColumn} (${sortDirection.toUpperCase()})`}</span>
            </span>
            {isFiltering && (
              <button
                type="button"
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 flex-1 min-w-0">
            <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
              <option value="ALL">All Types (Staff & Worker)</option>
              <option value="Staff">Staff</option>
              <option value="Worker">Worker</option>
            </select>
            <select value={companyFilter} onChange={(e) => setCompanyFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
            ...
            <select value={jobFilter} onChange={(e) => setJobFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 truncate font-medium">
              <option value="ALL">All Designations</option>
              {availableJobs.map((j) => (
                <option key={j} value={j}>{j}</option>
              ))}
            </select>
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
              <option value="ALL">All Statuses</option>
              <option value="Unpaid">Unpaid</option>
              <option value="Partially Paid">Partially Paid</option>
              <option value="Fully Paid">Fully Paid</option>
            </select>
            <select value={paidByFilter} onChange={(e) => setPaidByFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
            ...
            <select value={wpsFilter} onChange={(e) => setWpsFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
              <option value="ALL">WPS: All</option>
              <option value="Yes">WPS Employees</option>
              <option value="No">Non-WPS</option>
            </select>
            <select value={wageTypeFilter} onChange={(e) => setWageTypeFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
              <option value="ALL">All Wage Types</option>
              <option value="Per Hour">Per Hour</option>
              <option value="Fixed Monthly">Fixed Monthly</option>
            </select>
            <select value={receiptStatusFilter} onChange={(e) => setReceiptStatusFilter(e.target.value)} className="px-2.5 py-1.5 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs text-slate-700 dark:text-slate-300 focus:ring-2 focus:ring-emerald-500 font-medium">
          ...
          <div className="flex items-center gap-3">
            ...
            {sortColumn !== 'default' && (
              <button
                ...
                className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 font-medium cursor-pointer"
              >
                Reset Sort
              </button>
            )}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-xs overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-2 bg-slate-50 dark:bg-slate-800/60 border-b border-slate-200 dark:border-slate-700 text-xs text-slate-600 dark:text-slate-400">
          <div className="flex items-center gap-2">
            <span className="font-medium text-slate-700 dark:text-slate-300">Salary Payment Ledger</span>
        ...
        <div className="overflow-x-auto">
          ...
          >
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800 font-medium text-slate-700 dark:text-slate-300">
              {loading ? (
                <tr><td colSpan={13} className="px-6 py-10 text-center text-slate-400 dark:text-slate-500">Loading payment ledger...</td></tr>
                  ...
                  <td colSpan={13} className="px-6 py-10 text-center text-slate-400 dark:text-slate-500">
                    ...
                    {isFiltering && (
                      <button
                        type="button"
                        onClick={handleResetFilters}
                        className="text-xs mt-1 text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 font-semibold cursor-pointer"
                      >
                        Reset Filters
                      </button>
                    )}
                ...
                displayRows.map((row, idx) => {
                  ...
                  return (
                    <tr key={`${row.employeeId}_${row.payrollMonth}_${idx}`} className="hover:bg-slate-50/70 transition-colors">
                      {row.dynamicFirst && (
                        <>
                          <td className="px-3 py-3 text-slate-600 dark:text-slate-400 align-top" rowSpan={row.dynamicSpan}>{row.employeeCompany}</td>
                          <td className="px-3 py-3 text-slate-600 dark:text-slate-400 align-top" rowSpan={row.dynamicSpan}>{row.salaryPaidBy}</td>
                          <td className="px-3 py-3 text-center align-top" rowSpan={row.dynamicSpan}>
                            <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold ${
                              row.wpsEmployee === 'Yes' ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-800 dark:text-blue-300' : 'bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400'
                            }`}>
                              {row.wpsEmployee === 'Yes' ? 'WPS' : 'Non-WPS'}
                            </span>
                          </td>
                          <td className="px-4 py-3 align-top" rowSpan={row.dynamicSpan}>
                            <span className="font-mono font-bold text-blue-600 dark:text-blue-400 block">{row.employeeId}</span>
                            <span className="font-semibold text-slate-900 dark:text-slate-100">{row.employeeName}</span>
                          </td>
                        </>
                      )}
                      <td className="px-3 py-3 font-mono text-slate-600 dark:text-slate-400">{row.payrollMonth}</td>
                      ...
                      <td className="px-4 py-3 text-right font-mono font-bold">
                        <span className={row.outstanding > 0 ? 'text-rose-600 dark:text-rose-400' : 'text-slate-400 dark:text-slate-500'}>
                          ...
                          <div className="flex items-center justify-center gap-1">
                            {receiptedTx.map((tx: any) => (
                              <button
                                ...
                                className="p-1 rounded bg-emerald-50 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 hover:bg-emerald-100 dark:hover:bg-emerald-900/50 transition-colors"
                              >
                      ...
                      <td className="px-3 py-3 text-center">
                        <button
                          ...
                          className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded-md transition-colors cursor-pointer"
                        >
                      ...
                      <td className="px-4 py-3 text-slate-500 dark:text-slate-400">{latestTx?.remarks || '—'}</td>
                    </tr>
                  );
                })
              )}
      ...
      {isPayModalOpen && selectedRow && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden my-6">
            ...
            <form onSubmit={handleSavePayment} className="p-6 space-y-4">
              {duplicateWarning && (
                <div className="p-3 bg-amber-50 dark:bg-amber-900/30 border border-amber-300 dark:border-amber-700 rounded-lg text-xs text-amber-800 dark:text-amber-300 flex items-start gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-bold text-emerald-800 dark:text-emerald-300 focus:ring-2 focus:ring-emerald-500"
                  />
                  <span className="text-[10px] text-slate-400 dark:text-slate-500">Can be full or partial amount</span>
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-xs focus:ring-2 focus:ring-emerald-500"
                  />
                </div>
              </div>
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Bank Reference / Cheque No.</label>
                <input
                  type="text"
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl space-y-2">
                ...
                <input
                  ...
                  className="text-xs text-slate-500 dark:text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-emerald-100 file:text-emerald-800 hover:file:bg-emerald-200 cursor-pointer"
                />
                {payForm.receiptFileName && (
                  <p className="text-[11px] text-emerald-700 dark:text-emerald-300 font-medium">Attached: {payForm.receiptFileName}</p>
                )}
                <p className="text-[10px] text-slate-400 dark:text-slate-500">Optional. You can attach it later from Payment History.</p>
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-4xl overflow-hidden">
            ...
            <div className="p-6 space-y-4 max-h-[60vh] overflow-y-auto">
              {selectedEmployeeHistory.length === 0 ? (
                <p className="text-xs text-slate-400 dark:text-slate-500 italic">No disbursals recorded for this month yet.</p>
              ) : (
                      ...
                      {selectedEmployeeHistory.map((p) => (
                        <tr key={p.id} className={p.isReversed ? 'opacity-50' : ''}>
                          ...
                          <td className="px-3 py-2 text-slate-500 dark:text-slate-400">
                            {p.remarks || '—'}
                            {p.isReversed && <span className="ml-1.5 text-rose-600 dark:text-rose-400 font-semibold">(Reversed: {p.reversalReason})</span>}
                          </td>
                          ...
                          <td className="px-3 py-2 text-center">
                            {p.receiptStoragePath ? (
                              <button
                                ...
                                className="text-emerald-600 dark:text-emerald-400 hover:text-emerald-800 font-semibold inline-flex items-center gap-1"
                              >
                          ...
                          <td className="px-3 py-2 text-slate-500 dark:text-slate-400">{p.createdBy}</td>
                              ...
                              <div className="flex items-center justify-end gap-1">
                                {canEdit && (
                                  <button
                                    ...
                                    className="text-blue-600 dark:text-blue-400 hover:text-blue-800 font-semibold p-1 hover:bg-blue-50 dark:hover:bg-blue-900/40 rounded"
                                    title="Edit Payment"
                                  >
                                ...
                                {canReverse && (
                                  <button
                                    ...
                                    className="text-rose-600 dark:text-rose-400 hover:text-rose-800 font-semibold p-1 hover:bg-rose-50 dark:hover:bg-rose-900/40 rounded"
      ...
      {isEditModalOpen && editingPayment && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-xs p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden my-6">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50 dark:bg-slate-800/60">
              <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">Edit Payment Transaction</h3>
              <button
            ...
            <form onSubmit={handleSaveEdit} className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Amount (OMR) <span className="text-rose-500">*</span></label>
                  <input
                    type="number"
                    step="0.001"
                    required
                    ...
                    className="w-full px-3 py-2 border border-slate-200 dark:border-slate-700 rounded-lg text-xs font-mono font-bold text-emerald-800 dark:text-emerald-300 focus:ring-2 focus:ring-blue-500"
                  />
                </div>
                <div>
                  <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Payment Date <span className="text-rose-500">*</span></label>
                  <input
                    type="date"
                    required
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Pay To <span className="text-rose-500">*</span></label>
                <input
                  type="text"
                  required
              ...
              <div className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-xl space-y-2">
                ...
                <input
                  ...
                  className="text-xs text-slate-500 dark:text-slate-400 file:mr-3 file:py-1.5 file:px-3 file:rounded-md file:border-0 file:text-xs file:font-semibold file:bg-blue-100 file:text-blue-800 hover:file:bg-blue-200 cursor-pointer"
                />
                {editForm.receiptFileName && (
                  <p className="text-[11px] text-blue-700 dark:text-blue-300 font-medium">New file selected: {editForm.receiptFileName}</p>
                )}
              </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between bg-slate-50 dark:bg-slate-800/60">
              <div className="flex items-center gap-2">
                ...
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">Bulk Import Salary Payments</h3>
              </div>
              <button
                onClick={() => {
                  setIsImportModalOpen(false);
            ...
            <div className="p-6 space-y-4">
              {!importPreview ? (
                <div className="border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-8 text-center hover:border-emerald-500 transition-colors">
                  <FileSpreadsheet className="w-10 h-10 text-emerald-500 mx-auto mb-2" />
                  <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">Upload a Filled Payment Disbursal File</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Columns: Employee ID, Salary Month, Payment Date, Pay Amount, Pay To, Receipt Reference, Remarks</p>
                  <input
                    type="file"
                    accept=".xlsx,.xls"
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-slate-50 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 rounded-lg text-xs">
                    ...
                    <span className="text-purple-700 dark:text-purple-300">Duplicate: <strong>{importPreview.summary.duplicateCount}</strong></span> •{' '}
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                        {importPreview.rows.map((r: any, idx: number) => (
                          <tr key={idx} className={r.status === 'Invalid' ? 'bg-rose-50/50' : r.status === 'Duplicate' ? 'bg-purple-50/50' : ''}>
                            ...
                            <td className="px-3 py-2 text-slate-500 dark:text-slate-400 text-[11px]">{r.message}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
